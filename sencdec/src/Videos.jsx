...
function Videos() {
  ...
  const handleContentEncryption = () => {
    ...
    downloadTextFile(encryptedText, 'encrypted-video.txt');
  };
  ...
  const handleContentDecryption = () => {
    ...
    downloadVideoFromHex(decryptedText, 'decrypted-video.mp4'); // Download as video file
  };

  // Convert hex content back to video and download
  const downloadVideoFromHex = (hexContent, fileName) => {
    const byteArray = new Uint8Array(
      hexContent.match(/.{1,2}/g).map((byte) => parseInt(byte, 16))
    ); // Convert hex to byte array
    const blob = new Blob([byteArray], { type: 'video/mp4' }); // Create Blob with video MIME type
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = fileName;
            ...
            <div className="EncDecBoxes">
              ...
              <div className="EncBox">
                ...
                <div className="EncBoxFoot">
                  <label htmlFor="imageUpload">Upload Video</label>
                  <input
                    id="imageUpload"
                    type="file"
                    accept="video/*"
