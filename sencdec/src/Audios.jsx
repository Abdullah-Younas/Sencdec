...
function Audios() {
  ...
  const handleContentEncryption = () => {
    ...
    downloadTextFile(encryptedText, 'encrypted-audio.txt');
  };
  ...
  const handleContentDecryption = () => {
    ...
    downloadAudioFromHex(decryptedText, 'decrypted-audio.mp3'); // Download as audio file
  };

  // Convert hex content back to audio and download
  const downloadAudioFromHex = (hexContent, fileName) => {
    const byteArray = new Uint8Array(
      hexContent.match(/.{1,2}/g).map((byte) => parseInt(byte, 16))
    ); // Convert hex to byte array
    const blob = new Blob([byteArray], { type: 'audio/mpeg' }); // Create Blob with audio MIME type
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = fileName;
          ...
          <div className="EncDecBoxes">
            ...
            <div className="EncBox">
              ...
              <div className="EncBoxFoot">
                <label htmlFor="imageUpload">Upload Audio</label>
                <input
                  id="imageUpload"
                  type="file"
                  accept="audio/*"
